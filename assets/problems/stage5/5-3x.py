def get_destination_folder(file_info, rules):
    ...
    for rule in rules:
        if file_info['type'] in rule['types'] \
            or file_info['size_kb'] > rule['min_size_kb']:    # ❌ 오류 1: 파일 타입과 크기 조건이 모두 맞아야 하는데, 하나만 맞아도 규칙을 적용함
...
def create_move_plan(file_list, rules):
    ...
    for file_obj in file_list:
        ...
        move_plan['file_obj'] = generate_new_filepath(file_obj, destination) # ❌ 오류 3: 파일명을 키로 사용해야 하는데, 파일 객체 전체를 키로 사용하려 함
